...
# pipeline_gdrive_full_v4.py
# Colab-friendly: uploads outputs back to the SAME Drive folder as the shared link
# using Colab OAuth (no client_secrets.json required). Also works locally (pydrive2).

from __future__ import annotations
import re, sys, os, io, shutil
...
def download_from_drive_url(url: str) -> tuple[Path, str]:
    """
    Download a share link via gdown and return (local_path, file_id).
    """
...
def resolve_input_any(user_text: str) -> tuple[Path, str | None]:
    s = user_text.strip().strip('"').strip("'")
...
# ===================== Google Drive upload (same folder as input) =====================
def upload_outputs_back_same_folder_colab(file_id: str, local_paths: list[Path]) -> list[str]:
    """
    Colab-only uploader using google-api-python-client with Colab auth (no client_secrets.json).
    """
    try:
    ...
    try:
        ...
        import subprocess
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "google-api-python-client"])
        from googleapiclient.discovery import build
        from googleapiclient.http import MediaFileUpload

    drive_service = build("drive", "v3")

    # Get parent folder of the input file
...
def upload_outputs_back_same_folder_local(file_id: str, local_paths: list[Path]) -> list[str]:
    """
    Local fallback via pydrive2 (needs client_secrets.json on first run).
    """
    try:
        import pydrive2  # noqa
    except Exception:
        import subprocess
        subprocess.check_call([sys.executable, "-m", "pip", "install", "--quiet", "pydrive2"])
    ...
    uploaded = []
    for p in local_paths:
        gfile = drive.CreateFile({'title': p.name, **({'parents':[{'id': parent_id}]} if parent_id else {})})
...
def sniff_filetype_and_fix_extension(path: Path) -> Path:
    ...
    with open(path, "rb") as f:
        head = f.read(4096)
    if head[:2] == b"PK":
        newp = path.with_suffix(".xlsx"); path.rename(newp)
        print(f"ℹ️ No extension; inferred XLSX and renamed to: {newp.name}")
        return newp
    text = head.decode("utf-8", errors="ignore").lstrip()
    if text.startswith("{") or text.startswith("["):
        newp = path.with_suffix(".json"); path.rename(newp)
        print(f"ℹ️ No extension; inferred JSON and renamed to: {newp.name}")
        return newp
    newp = path.with_suffix(".csv"); path.rename(newp)
    print(f"ℹ️ No extension; inferred CSV and renamed to: {newp.name}")
    return newp
...
def unique_from_duplicates(input_duplicate_csv_path: Path) -> Path:
    ...
    print("\n" + "="*100)
    print("FINAL UNIQUE DUPLICATE ROWS".center(100))
    print("="*100 + "\n")
...
if __name__ == "__main__":
    # Prompt without emoji (Windows terminals can choke on it)
    try:
    ...
    # 1) Resolve input (and capture Drive file id if a link)
    in_path, drive_file_id = resolve_input_any(user_in)
    if not in_path.exists():
        raise SystemExit(f"File not found: {in_path}")

    # 2) In Colab: if input isn't under Drive, copy into Drive so all outputs land in Drive
    if in_colab() and not str(in_path).startswith("/content/drive"):
        ...
        print(f"Copied working input to Drive for local outputs: {new_path}")
        in_path = new_path

    # 3) Run the pipeline (saves outputs next to working input)
    base_csv, modules_csv, dups_csv, unique_csv = run_pipeline(in_path)

    # 4) Upload outputs to the SAME shared folder (if input was a Drive link)
    if drive_file_id:
        try:
            if in_colab():
                uploaded_ids = upload_outputs_back_same_folder_colab(
                    drive_file_id, [base_csv, modules_csv, dups_csv, unique_csv]
                )
            else:
            ...
            for p, fid in zip([base_csv, modules_csv, dups_csv, unique_csv], uploaded_ids):
                print(f"  - {p.name}  (file id: {fid})")
        except Exception as e:
            print(f"\n⚠️ Upload back to the shared file's folder failed: {e}")
            print("Outputs are still saved in your Drive next to the working input path shown above.")
